...
export default function Customers() {
  ...
  const handleAddPurchase = async (customerId) => {
    if (!purchaseForm.itemName || !purchaseForm.price) {
      setPurchaseMsg('Item name and price are required'); return;
    }
    try {
                    ...
                    {c.purchases && c.purchases.length > 0 && (
                      <div style={{ textAlign:'center', marginRight:'8px' }}>
                        ...
                        <div style={{ fontSize:'14px', fontWeight:'600', color:'#2d7a4f' }}>
                          ₹{totalSpent(c.purchases).toLocaleString('en-IN')}
                        </div>
                          ...
                          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:'13px' }}>
                            ...
                            <tbody>
                              {c.purchases.map((p, i) => (
                                <tr key={p._id} style={{ borderBottom:'1px solid #e2e0d8' }}>
                                  ...
                                  <td style={{ padding:'8px 10px', textAlign:'right' }}>₹{p.price.toLocaleString('en-IN')}</td>
                                  <td style={{ padding:'8px 10px', textAlign:'right' }}>{p.qty}</td>
                                  <td style={{ padding:'8px 10px', textAlign:'right', fontWeight:'600', color:'#2d7a4f' }}>₹{(p.price*p.qty).toLocaleString('en-IN')}</td>
                            ...
                            <tfoot>
                              <tr style={{ background:'#fdf0ed' }}>
                                ...
                                <td style={{ padding:'8px 10px', textAlign:'right', fontWeight:'700', color:'#c84b31', fontSize:'14px' }}>₹{totalSpent(c.purchases).toLocaleString('en-IN')}</td>
                                <td colSpan="2"></td>
                              </tr>
                            </tfoot>
                          </table>
