...
import { getProducts, getDeletedProducts, addProduct, deleteProduct, restoreProduct } from '../api';
...
const btn = (variant='default') => ({
  ...
  background: variant==='primary' ? '#c84b31' : variant==='danger' ? '#fdf0ee' : variant==='restore' ? '#edf7f2' : '#fff',
  color: variant==='primary' ? '#fff' : variant==='danger' ? '#c0392b' : variant==='restore' ? '#2d7a4f' : '#1a1917',
  cursor:'pointer'
});
...
export default function Products() {
  ...
  const load = async () => {
    const [active, deleted] = await Promise.all([getProducts(), getDeletedProducts()]);
    setProducts(active.data);
    setDeletedProducts(deleted.data);
  };
  useEffect(() => { load(); }, []);

  const handleAdd = async () => {
    if (!form.name || !form.actualPrice || !form.sellingPrice || !form.stock) {
    ...
    <div>
      ...
      <div style={{ display:'flex', gap:'8px', marginBottom:'16px' }}>
        <button onClick={()=>setTab('active')} style={{
          padding:'8px 20px', borderRadius:'20px', fontSize:'13px', fontWeight:'500', cursor:'pointer',
          border:'none',
          background: tab==='active' ? '#c84b31' : '#f5f4f0',
          color: tab==='active' ? '#fff' : '#6b6960'
        }}>
          Active Products ({products.length})
        </button>
        <button onClick={()=>setTab('deleted')} style={{
          padding:'8px 20px', borderRadius:'20px', fontSize:'13px', fontWeight:'500', cursor:'pointer',
          border:'none',
          background: tab==='deleted' ? '#c84b31' : '#f5f4f0',
          color: tab==='deleted' ? '#fff' : '#6b6960'
        }}>
      ...
      <div style={{ background:'#fff', border:'1px solid #e2e0d8', borderRadius:'12px', padding:'20px' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'16px' }}>
          <h2 style={{ fontSize:'15px', fontWeight:'600' }}>
            {tab === 'active' ? `Inventory (${filtered.length})` : `Deleted Products (${filteredDeleted.length})`}
          </h2>
        ...
        {tab === 'active' && (
          filtered.length === 0
            ...
            : <table style={{ width:'100%', borderCollapse:'collapse', fontSize:'13px' }}>
                <thead>
                  <tr>{['Name','Category','Purchase Price','Selling Price','Profit','Stock','Status',''].map(h=>(
                    <th key={h} style={{ textAlign:'left', padding:'8px 10px', color:'#6b6960', borderBottom:'1px solid #e2e0d8', fontSize:'12px' }}>{h}</th>
                  ))}</tr>
                </thead>
                <tbody>
                  {filtered.map(p => {
        ...
        {tab === 'deleted' && (
          filteredDeleted.length === 0
            ? <p style={{ color:'#6b6960', textAlign:'center', padding:'2rem' }}>No deleted products</p>
            : <table style={{ width:'100%', borderCollapse:'collapse', fontSize:'13px' }}>
                <thead>
                  <tr>{['Name','Category','Purchase Price','Selling Price','Stock','Deleted At',''].map(h=>(
                    <th key={h} style={{ textAlign:'left', padding:'8px 10px', color:'#6b6960', borderBottom:'1px solid #e2e0d8', fontSize:'12px' }}>{h}</th>
                  ))}</tr>
                </thead>
                <tbody>
                  {filteredDeleted.map(p => (
                    <tr key={p._id} style={{ borderBottom:'1px solid #f5f4f0', opacity:0.7 }}>
                      <td style={{ padding:'10px' }}><strong>{p.name}</strong></td>
                      <td style={{ padding:'10px', color:'#6b6960' }}>{p.category}</td>
                      <td style={{ padding:'10px' }}>₹{(p.actualPrice||0).toLocaleString('en-IN')}</td>
                      <td style={{ padding:'10px' }}>₹{(p.sellingPrice||0).toLocaleString('en-IN')}</td>
                      <td style={{ padding:'10px' }}>{p.stock}</td>
                      <td style={{ padding:'10px', color:'#6b6960', fontSize:'12px' }}>
                        {p.deletedAt ? new Date(p.deletedAt).toLocaleDateString('en-IN') : '-'}
                      </td>
                      <td style={{ padding:'10px' }}>
                        <button style={btn('restore')} onClick={()=>handleRestore(p._id)}>↩ Restore</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
